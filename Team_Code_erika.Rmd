...
```{r}
rap_hom <- usnat2 %>% filter(year1 >= '1960' & year1 <= '2012')%>% mutate(murderpct = murder/residents*1000, rapepct = forcible_rape/residents*1000)
...
ushousehold <- rap_hom %>% mutate (single_householdpct =((male_household + fem_household)/residents)*1000)
...
ushousehold <- ushousehold %>% mutate(housepct = (couple_household/residents)*1000) 
...
y<-ushousehold%>%select(tot_male, residents,year1)%>% mutate(total = (tot_male/residents)*100)%>% filter(year1>=2000)%>%mutate(key="tot_male")
x<-ushousehold%>%select(tot_female,residents,year1)%>% mutate(total = tot_female/residents*100)%>% filter(year1>=2000)%>%mutate(key="tot_female")
...
ggplot(xy, aes(year1,group=key,col=key))  + geom_point(aes(y=murderpct))+ geom_point(aes(y=total))+  geom_point(aes(y=rapepct, color= 'red')) +labs(title="U.S. Gender Trends vs murder", y = "Rate per 100000", x= "year")
...
##ggplot(ushousehold, aes(x=single_household, y=murder)) + geom_point( aes(color=year1 , size=residents)) 


##race <- c("white_male_pop", "white_female_pop", "black_male_pop", "black_female_pop", "his_male_pop", "his_female_pop")



##p <- ggplot( filter(ushosehold, year1>=1988), aes(x=year1, y=murder))
##p + geom_point(aes( color=race, size=residents)) 
