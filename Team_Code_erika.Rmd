...
rap_hom  %>% ggplot(aes(year1)) + geom_point(aes(y = murderpct, col = "Murder")) + geom_point(aes(y=rapepct, col="Rape")) + labs(title="U.S. Forcible Rape and Murder Trends", y = "Rate per 100000", col="Values")
...
dat %>% ggplot(aes(year1)) + geom_point(aes(y=rapepct)) + geom_line(aes(x=year1, y=.fitted), data=loess_fit, color="red") + labs(title="Rape Trend - Loess")
...
dat %>% ggplot(aes(year1)) + geom_point(aes(y=murderpct)) + geom_line(aes(x=year1, y=.fitted), data=loess_fit, color="red") + labs(title="Murderpct Trend - Loess")
...
ushousehold <- rap_hom %>% mutate (single_householdpct =((male_household + fem_household)/residents)*10000)
...
ggplot(ushousehold, aes(x=single_householdpct, y=murderpct)) + geom_point( aes(color=year1 , size=residents)) +labs(title="U.S. Murder Trends in single household", y = "Murder percentage")
...
hist_pop <- data.frame(year1,under18,under44,over65)
#hist_pop$year1 <- as.character(hist_pop$year1)
kable(hist_pop)

## Plotting it against the rape curve
age<-ushousehold%>% left_join(hist_pop) 

age%>%ggplot(aes(year1)) + geom_point(aes(y=under44, color = "% Median Age"))  + geom_point(aes(y=under18, color = "% Under 18"))  +  geom_point(aes(y=over65, color = " % Over 65"))  +
geom_point(aes(y=murderpct, color = " % Murder"))+ geom_point(aes(y = rapepct,color=" % Rape ")) +labs(title="Rape and Murder vs Age", y = "Rate per 100000")
...
```{r}
y<-ushousehold%>%select(tot_male, residents,year1)%>% mutate(total = tot_male/residents*10000)%>% filter(year1>=2000)%>%mutate(key="tot_male")
x<-ushousehold%>%select(tot_female,residents,year1)%>% mutate(total = tot_female/residents*10000)%>% filter(year1>=2000)%>%mutate(key="tot_female")
...
```{r}
ushousehold%>% ggplot(aes(year1)) + geom_point(aes(y=inc_median_all_male, col="Male")) + geom_point(aes(y=inc_median_all_female, col="Female"))+labs(title="U.S. Income Men vs Female", y = "Rate per 100000", x= "year", col="values")
 
income<-rap_hom%>%filter(year1<=2009)%>%mutate(total_income=(inc_median_all_male+inc_median_all_female)/2)
...
```{r}
ushousehold %>% ggplot(aes(year1))+ geom_point(aes(y=forcible_rape, col="forcible_rape")) +geom_point(aes(y=murder, col="murder")) + geom_point(aes(y=residents/1000, col="population")) + labs(title="U.S. Population, Rape and Homocide", y = "Rate per 100000", x= "year", col="values")
```
