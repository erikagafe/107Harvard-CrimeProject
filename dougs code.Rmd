...
As check, we take a look at post 2009 data and see if it fits at all. 

First, we take a look at the states for 2009.

```{r}
st_rape_pct_t <- st_rape_pct %>% filter(y == '2009')
st_rape_pct_t <- as.matrix(st_rape_pct_t)
st_rape_pct_t <- t(st_rape_pct_t)
st_rape_pct_t <- as.data.frame(st_rape_pct_t)
st_rape_pct_t$V1 <- as.double(st_rape_pct_t$V1)


```

So this gives a list of the states with the highest percentage. For this, we go on to several packages that pull from the UScensus data. These are
